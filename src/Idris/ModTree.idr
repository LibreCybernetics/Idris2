...
buildMod loc num len mod
   = do clearCtxt; addPrimitives
        ...
        let sourceFileName = buildFile mod
        ...
        codeHash            <- hashFile sourceFileName
        (storedCodeHash, _) <- readHashes ttcFileName
        ...
        -- file, have been modified from the stored hashes
        let needsBuilding = (codeHash /= storedCodeHash || depFilesHashDiffers)
           ...
           do let pad = minus (length $ show len) (length $ show num)
              let msg : Doc IdrisAnn
                  = pretty num <+> pretty (String.replicate pad ' ')
                    ...
                    <++> parens (pretty sourceFileName)
              process {u} {m} msg sourceFileName modNamespace
